...
def train_pinn(data_path, layers, epochs=200000, batch_size=5000, lr=0.001, noise_level=0.0, save_path="model.pth", track_loss=False):
    ...
    for epoch in range(epochs):
        ...
        loss = model.loss_function(u_train, v_train, (u_pred, v_pred, p_pred, f_u_pred, f_v_pred))
        loss.backward()
        optimizer.step()
        ...
        if track_loss:
            ...
            val_loss = model.loss_function(u_val, v_val, (u_val_pred, v_val_pred, p_val_pred, f_u_val_pred, f_v_val_pred))
            model.train()
